...
# Волна 4 — полная групповая атака (6 агентов) + фиксы

Запущено 6 red-team агентов (мой новый код, публичная бронь, клиентское доверие, промо/рефералы/цены, offline/iCal/storage, полный критик). Итог: подтверждён корень C1 + найдены **новые не-C1 дыры**, чинимые сразу.

## ✅ Исправлено и задеплоено (не-C1)
- **CRITICAL zero-to-super**: `authenticateSuper` больше не читает `superPassHash` из анонимно-записываемого `settings`; только Secret Manager. (Супер сейчас отключён — если нужен, задать секрет `SUPER_PASSWORD_HASH` + привязать.)
- **CRITICAL реферальный фрод**: `confirmTenDayStay`/`addBonusDays` — админ-гейт + кап + аудит (кассир мутил бесплатные ночи фейк-рефералами).
- **CRITICAL самоодобрение скидки**: `handleGrantPriceReduction/Revoke` — админ-гейт.
- **HIGH вторая утечка PII**: `occupancy-widget.html` (отдельный виджет, пропущен в C6) переведён на `getPublicAvailability`.
- **HIGH `amountPaid: NaN`** ломал отчёт долгов/Excel — Number.isFinite-гвард.
- **MEDIUM (моя регрессия)**: admin-stats throttle снова с глобальным капом (спуфинг XFF давал безлимитный перебор).
- **MEDIUM**: `getPublicAvailability` — только активные промо, минимум полей; `recurringExpenses` детерминированный id (нет двойной зарплаты) + NaN-гвард; `storage.rules` content-type allowlist (без svg-XSS) + `logos/`; `functions/` npm audit fix (1 critical + 4 high устранены).

## ⚠️ Ещё открыто (не-C1, отдельными шагами)
- **`siteCallbackKey`** (`hst-cb-a81f37c2d94e`) захардкожен в бандле → форж callback на `hostella.uz`. Нужна ротация ключа + HMAC на стороне PHP-сайта (твоё действие + правка сайта).
- **`pending_payments.json`**: рендерер-записываемый файл может подсунуть поддельное «Оплата принята» в Telegram (HTML уже режется сервером, но форму записи надо валидировать / не хранить telegram-записи на диске) — Electron, следующая сборка.
- **e-mehmon занижение налога** (E1: иностранец как «Узбекистан»; E2: сумма по умолчанию «1») — логика отчётности.
- **CF1**: скрытые списания не видны в бригадных/сменных отчётах (аудит добавлен, но в отчёты не выведены).
- **P4**: записывается введённая цена, а не whitelist-цена (клампинг).

## 🔴 Корень C1 (замок правил) — пререквизиты от агента #6 (критично для безопасного деплоя)
1. Правила писать под claim `hostellaRole`/`hostellaHostel` (НЕ isStaff/isAdmin).
2. Токены минтятся (IAM-роль выдана ✓ — проверено).
3. `useAppData` вешает слушатели ДО входа (на аноним) → гейтить на `currentUser`, иначе слушатели упадут на экране входа.
4. `settings/appConfig` держать читаемым (нужен до входа).
