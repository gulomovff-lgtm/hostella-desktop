...
const NodeActions = ({ node, settings, guests = [], t = (k) => k, onConfirm, onRedeem, onAddBonus, onResetBonus, onExtendStay, onRemove }) => {
    ...
    <div className="space-y-2.5">
      ...
      {!node.referralConfirmed && (
        <button type="button" onClick={() => onConfirm(node.id)}
          ...
          <CheckCircle2 size={14} /> {t('rfConfirmStayDays').replace('{n}', settings?.minStayDays || 10)}
        </button>
      )}
      {node.referralConfirmed && !hasBonus && (
        <p className="text-center text-[11px] text-emerald-600 font-semibold py-1">{t('rfStayConfirmed')}</p>
      )}

      {/* Redeem bonus */}
      {hasBonus && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl p-2">
          <span className="text-[11px] text-amber-700 font-bold ml-1 shrink-0">{t('rfRedeemLabel')}</span>
          ...
          <button type="button" onClick={() => onRedeem(node.id, redeemAmt)}
            ...
            {t('rfRedeemDays').replace('{n}', redeemAmt)}
          </button>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        <span className="text-[11px] text-slate-500 font-semibold shrink-0">{t('rfAccrueLabel')}</span>
        ...
        <button type="button" onClick={() => onAddBonus?.(node.id, addAmt)}
          ...
          <Gift size={12} /> {t('rfPlusDays').replace('{n}', addAmt)}
        </button>
        <button type="button" onClick={() => onResetBonus?.(node.id)} title={t('rfResetBonuses')}
      ...
      {hasBonus && (
        <div>
          <button type="button" onClick={() => setShowExtend(v => !v)}
            ...
            <Sparkles size={12} /> {t('rfExtendWithBonus')} {showExtend ? '▲' : '▼'}
          </button>
          {showExtend && (
              ...
              {!selectedGuest ? (
                <div className="space-y-1">
                  <input value={extSearch}
                    ...
                    placeholder={t('rfSearchActiveGuest')}
                  ...
                  {searchGuests.length === 0 && (
                    <p className="text-[10px] text-slate-400 text-center py-1">{t('rfNoActiveGuests')}</p>
                  )}
                  <div className="space-y-0.5 max-h-24 overflow-y-auto">
                    {searchGuests.map(g => (
                      <button key={g.id} type="button"
                        ...
                        <span className="ml-1.5 text-slate-400">{t('rfCheckout')} {fmtDate(g.bonusCheckOutDate || g.checkOutDate)}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-slate-400">{t('rfCheckout')} {fmtDate(selectedGuest.bonusCheckOutDate || selectedGuest.checkOutDate)}</span>
                  <button type="button" onClick={() => { setExtGuestId(''); setExtSearch(''); }}
                    className="text-slate-400 hover:text-slate-600 ml-1"><X size={11} /></button>
                </div>
              )}
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-slate-500 shrink-0">{t('rfDaysColon')}</span>
                ...
                <span className="text-[10px] text-slate-400">{t('rfMax').replace('{n}', node.bonusDays)}</span>
              </div>
      ...
      {!confirmRemove ? (
        <button type="button" onClick={() => setConfirmRemove(true)}
          ...
          <Trash2 size={12} /> {t('rfRemoveFromProgram')}
        </button>
      ) : (
        <div className="rounded-xl bg-rose-50 border border-rose-200 px-3 py-2.5 space-y-2">
          <p className="text-xs text-rose-600 text-center font-semibold">{t('rfRemoveConfirm').replace('{name}', displayName)}</p>
          <div className="flex gap-2">
            <button type="button" onClick={() => { onRemove(node.id); setConfirmRemove(false); }}
              className="flex-1 py-1.5 rounded-xl bg-rose-500 hover:bg-rose-400 text-white text-xs font-bold transition-colors">{t('rfYesRemove')}</button>
            <button type="button" onClick={() => setConfirmRemove(false)}
              className="flex-1 py-1.5 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-600 text-xs font-medium transition-colors">{t('cancel')}</button>
          </div>
        </div>
      )}
    </div>
...
const TreeNodeCard = ({ node, level, selectedId, onSelect, settings, guests, handlers, t = (k) => k }) => {
    ...
    <div className={`relative rounded-2xl transition-all duration-200 select-none ${isVirtual ? 'w-56' : 'w-52'}`}>
        ...
        <div className={`px-3.5 pt-3 pb-3 bg-gradient-to-br ${col.grad} relative`}>
          ...
          {hasBonus && (
            <div className="absolute top-2 right-2 bg-white text-amber-600 text-[10px] font-black px-1.5 py-0.5 rounded-full shadow-sm flex items-center gap-0.5">
              <Gift size={9} /> {node.bonusDays}{t('rfDayShort')}
            </div>
          )}
        </div>
        {/* body */}
          ...
          <div className="px-3.5 py-2.5 space-y-2">
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] text-slate-400">{t('rfToBonus')}</span>
            ...
            <div className="flex items-center justify-between text-[10px] text-slate-400">
              <span>👥 {(node.children || []).length} {t('rfInvited')}</span>
      ...
      {isSelected && !isVirtual && (
        <div className="mt-1.5 rounded-2xl bg-white border border-slate-200 p-3 shadow-lg" onClick={e => e.stopPropagation()}>
          <NodeActions node={node} settings={settings} guests={guests} t={t} {...handlers} />
        </div>
      )}
      {isSelected && isVirtual && (
...
/* ─── Tree branch ─────────────────────────────────────────────────── */
const TreeBranch = ({ node, level = 0, selectedId, onSelect, settings, guests, handlers, t = (k) => k }) => {
  const hasChildren = (node.children?.length || 0) > 0;
  const col = lc(level);
  const childCol = lc(level + 1);
  const shared = { selectedId, onSelect, settings, guests, handlers, t };
...
const ListRow = ({ node, level, expanded, onToggle, settings, guests, handlers, t = (k) => k }) => {
  const col = lc(level);
  const hasBonus = (node.bonusDays || 0) > 0;
  const tiersCount = settings?.tiers?.length || 2;
  const made = node.referralsMade || 0;
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <button type="button" onClick={onToggle} className="flex-1 min-w-0 text-left group">
          ...
          <div className="text-[11px] text-slate-400 mt-0.5 flex items-center gap-2 flex-wrap">
            {node.referralConfirmed
              ? <span className="text-emerald-600 font-medium inline-flex items-center gap-0.5"><CheckCircle2 size={10} /> {t('rfConfirmedLower')}</span>
              : <span className="text-amber-600 font-medium inline-flex items-center gap-0.5"><Clock size={10} /> {t('rfPending')}</span>}
            <span>👥 {invited}</span>
          </div>
          {/* Прогресс до бонуса */}
        ...
        {hasBonus && (
          <div className="shrink-0 flex items-center gap-1 bg-amber-50 border border-amber-200 rounded-xl px-2.5 py-1.5">
            ...
            <span className="text-[9px] font-bold text-amber-400">{t('rfDaysAbbr2')}</span>
          </div>
        )}
        ...
        <div className="shrink-0 flex items-center gap-1.5">
          {!node.referralConfirmed && (
            <button type="button" onClick={() => handlers.onConfirm(node.id)}
              title={t('rfConfirmStayDays').replace('{n}', settings?.minStayDays || 10)}
              ...
              <CheckCircle2 size={13} /> <span className="hidden sm:inline">{t('confirm')}</span>
            </button>
          )}
          <button type="button" onClick={() => handlers.onAddBonus?.(node.id, 1)}
            title={t('rfAccrueOneBonus')}
...
const AddPanel = ({ participantList, nonParticipants, onAddNew, onLinkExisting, t = (k) => k }) => {
  ...
  return (
    <div className="space-y-3">
      <div className="flex rounded-xl overflow-hidden border border-slate-200 bg-slate-100 p-1 gap-1">
        {[['new', t('rfNewGuest')], ['existing', t('rfFromBase')]].map(([m, label]) => (
      ...
      {mode === 'new' ? (
        <form onSubmit={handleNew} className="space-y-2.5">
          <div>
            <label className="text-[11px] text-slate-500 font-semibold block mb-1">{t('rfGuestName')}</label>
            <input ref={inputRef} value={name} onChange={e => setName(e.target.value)} placeholder={t('rfEnterName')} required className={inputCls} />
          </div>
          <div>
            <label className="text-[11px] text-slate-500 font-semibold block mb-1">{t('rfInvitedBy')}</label>
          ...
          <button type="submit" className="w-full py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold shadow-sm shadow-teal-200 transition-all active:scale-95 flex items-center justify-center gap-2">
            <UserPlus size={15} /> {t('rfAddGuest')}
          </button>
        </form>
      ) : (
        <form onSubmit={handleLink} className="space-y-2.5">
          <div>
            <label className="text-[11px] text-slate-500 font-semibold block mb-1">{t('rfSearchClient')}</label>
            <div className="relative">
              <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
              <input value={search} onChange={e => { setSearch(e.target.value); setPickedId(''); }} placeholder={t('rfFullName')} className={`${inputCls} pl-8`} />
            </div>
            ...
            {search && filteredNP.length === 0 && <p className="text-[10px] text-slate-400 mt-1 px-1">{t('rfNotFound')}</p>}
          </div>
          <div>
            <label className="text-[11px] text-slate-500 font-semibold block mb-1">{t('rfInvitedBy')}</label>
...
const RulesCard = ({ settings, t = (k) => k }) => {
  const rules = settings?.customRules?.filter(r => r.text?.trim()) || [];
  const tiers = settings?.tiers || [];
  const minDays = settings?.minStayDays || 10;
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 shadow-sm">
      <p className="text-xs font-bold text-slate-700 flex items-center gap-1.5"><Info size={13} className="text-teal-500" /> {t('rfHowWorks')}</p>
      {tiers.length > 0 && (
        <div className="space-y-1.5">
          {tiers.map((tier, i) => (
            <div key={tier.id} className="flex items-center justify-between bg-slate-50 rounded-lg px-2.5 py-1.5">
              <span className="text-[11px] text-slate-500"><span className="font-bold text-slate-700">{i + 1}.</span> {tier.label}</span>
              <span className="text-[11px] font-black text-amber-600">{t('rfPlusDays').replace('{n}', tier.bonusDays)}</span>
            </div>
          ))}
          {settings?.resetAfterCycle && <p className="text-[10px] text-slate-400 px-1">{t('rfResetAfterCycle')}</p>}
        </div>
      )}
      {rules.length > 0 && (
      ...
      <p className="text-[10px] text-slate-400 border-t border-slate-100 pt-2">
        {t('rfMinStay')} <span className="text-slate-700 font-semibold">{minDays} {t('rfDaysAbbr2')}</span>
        {settings?.maxBonusDays > 0 && <> · {t('rfLimit')} <span className="text-slate-700 font-semibold">{settings.maxBonusDays} {t('rfDDot')}</span></>}
        {settings?.bonusExpiryDays > 0 && <> · {t('rfExpireIn')} <span className="text-slate-700 font-semibold">{settings.bonusExpiryDays} {t('rfDaysAbbr2')}</span></>}
      </p>
    </div>
  );
};
...
const SettingsModal = ({ settings, onClose, onSave, saving, patchSettings, addTier, updateTier, removeTier, moveTier, addRule, updateRule, removeRule, t = (k) => k }) => {
  const [tab, setTab] = useState('general');
  const TABS = [
    { id: 'general', label: t('rfTabGeneral') },
    { id: 'tiers', label: t('rfTabTiers') },
    { id: 'rules', label: t('rfTabRules') },
  ];
    ...
    <div className="modal-centered fixed inset-0 z-[200] flex items-center justify-center p-4 pb-[84px] sm:pb-4 bg-slate-900/50 backdrop-blur-sm"
      ...
      <div className="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl shadow-2xl flex flex-col" style={{ maxHeight: '88vh' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">{t('rfSettingsTitle')}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {tab === 'general' && (
            <div className="space-y-4">
              <label className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-2xl cursor-pointer">
                <div>
                  <p className="text-sm font-semibold text-slate-700">{t('rfProgramActive')}</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">{t('rfProgramActiveDesc')}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className={lbl}>{t('rfMaxBonus')}</label>
                ...
                <div>
                  <label className={lbl}>{t('rfBonusExpiry')}</label>
                ...
                <label className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer">
                  ...
                  <span className="text-[11px] text-slate-600 leading-tight">{t('rfResetCounterTiers')}</span>
                </label>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              {settings.tiers.map((tier, idx) => (
                <div key={tier.id} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl p-3">
                  ...
                  <input value={tier.label} onChange={e => updateTier(tier.id, { label: e.target.value })} placeholder={t('rfTierName')}
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] text-slate-400 whitespace-nowrap">{t('rfPlusDaysLabel')}</span>
          ...
          {tab === 'rules' && (
            <div className="space-y-3">
              <p className="text-[11px] text-slate-400">{t('rfRulesDesc')}</p>
              {settings.customRules.map((rule, idx) => (
                <div key={rule.id} className="flex gap-2 items-start">
                  ...
                  <textarea value={rule.text} rows={2} onChange={e => updateRule(rule.id, e.target.value)} placeholder={t('rfRuleText')}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 shrink-0">
          <button onClick={onClose} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 text-sm transition-colors">{t('cancel')}</button>
          <button onClick={() => onSave(settings)} disabled={saving}
            ...
            <Save size={14} />{saving ? t('rfSaving') : t('save')}
          </button>
        </div>
      </div>
    </div>
...
const ReferralView = ({ clients = [], guests = [], hostelId, showNotification, currentUser, lang = 'ru' }) => {
  ...
  const hostelBadge = hostelId && hostelId !== 'all'
    ? (hostelId === 'hostel1' ? t('staffHostel1') : hostelId === 'hostel2' ? t('staffHostel2') : hostelId)
    : null;

  const FILTERS = [
    { id: 'all',       label: t('rfAll'),             count: filterCounts.all },
    { id: 'bonus',     label: t('rfWithBonus'),       count: filterCounts.bonus },
    { id: 'pending',   label: t('rfFilterPending'),   count: filterCounts.pending },
    { id: 'confirmed', label: t('rfFilterConfirmed'), count: filterCounts.confirmed },
  ];
    ...
    <div className="min-h-full bg-slate-50 flex flex-col">
      ...
      <div className="shrink-0 px-4 md:px-6 pt-4 pb-3 bg-white border-b border-slate-200">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg font-black text-slate-800 leading-tight truncate flex items-center gap-2">
                ...
                {!settings.programActive && <span className="text-[10px] bg-rose-50 text-rose-500 border border-rose-200 px-2 py-0.5 rounded-full font-semibold shrink-0">{t('rfDisabled')}</span>}
              </h1>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setInfoOpen(true)}
              ...
              <Info size={14} /> <span className="hidden sm:inline">{t('rfHowWorks')}</span>
            </button>
            {isAdmin && (
              <button onClick={() => setShowSettings(true)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-slate-200 hover:border-teal-300 hover:text-teal-600 text-slate-500 text-xs font-semibold transition-all shadow-sm active:scale-95">
                <Settings size={14} /> <span className="hidden sm:inline">{t('rfSettings')}</span>
              </button>
            )}
            <button onClick={() => setAddOpen(true)}
              ...
              <UserPlus size={14} /> {t('rfParticipant')}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap mt-3">
          {[
            { icon: Users,        value: stats.totalGuests,      label: t('rfParticipantsStat'), tint: 'bg-teal-50 text-teal-700 border-teal-200' },
            { icon: CheckCircle2, value: stats.confirmedGuests,  label: t('rfConfirmedStat'),    tint: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
            { icon: Clock,        value: stats.pendingGuests,    label: t('rfPendingStat'),      tint: 'bg-amber-50 text-amber-700 border-amber-200' },
            { icon: Gift,         value: stats.totalBonusPending, label: t('rfBonusAvailStat'),  tint: 'bg-orange-50 text-orange-700 border-orange-200' },
            { icon: Award,        value: stats.totalBonusEarned, label: t('rfEarnedStat'),       tint: 'bg-violet-50 text-violet-700 border-violet-200' },
      ...
      <div className="shrink-0 px-4 md:px-6 py-2.5 flex items-center gap-2 flex-wrap border-b border-slate-200 bg-white/80 backdrop-blur-sm">
        ...
        {view === 'list' && (
          <div className="relative flex-1 min-w-[140px] max-w-xs">
            <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input value={listSearch} onChange={e => setListSearch(e.target.value)} placeholder={t('rfSearchDots')}
        ...
        {view === 'tree' && (
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl px-2 py-1 shadow-sm">
            ...
            {treeScale !== 1 && <button onClick={() => setTreeScale(1)} className="ml-1 px-2 h-6 flex items-center rounded-lg hover:bg-slate-100 text-slate-400 text-[10px]">{t('rfResetShort')}</button>}
          </div>
        )}
        {view === 'tree' && (
          <div className="hidden md:flex items-center gap-2.5 px-2">
            {LEVEL_COLORS.map((c, i) => (
              <span key={i} className="flex items-center gap-1 text-[10px] text-slate-400">
                <span className={`w-2 h-2 rounded-sm ${c.dot}`} /> {i === 0 ? t('rfRoot') : t('rfLevelN').replace('{n}', i)}
              </span>
            ))}
          </div>
        )}
        ...
        <div className="ml-auto flex rounded-xl overflow-hidden border border-slate-200 bg-slate-100 p-1 gap-1">
          {[['list', t('rfList'), ListIcon], ['tree', t('rfTree'), Network]].map(([v, label, Icon]) => (
      ...
      <div ref={canvasRef} onTouchMove={view === 'tree' ? handleTouchMove : undefined} onTouchEnd={view === 'tree' ? handleTouchEnd : undefined}
        ...
        {!hasParticipants ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-500 font-semibold text-sm">{t('rfNoParticipants')}</p>
            <button onClick={() => setAddOpen(true)}
              ...
              <UserPlus size={15} /> {t('rfAddFirst')}
            </button>
          </div>
        ) : view === 'list' ? (
          <div className="max-w-3xl mx-auto space-y-2">
            {flatList.length === 0 ? (
              <p className="text-center text-slate-400 text-sm py-10">{t('nothingFound')}</p>
            ) : flatList.map(({ node, level }) => (
              <ListRow key={node.id} node={node} level={level}
                expanded={expandedId === node.id}
                onToggle={() => setExpandedId(id => id === node.id ? null : node.id)}
                settings={settings} guests={guests} handlers={handlers} t={t} />
            ))}
          </div>
        ) : (
          <div style={{ transform: `scale(${treeScale})`, transformOrigin: 'top left', transition: 'transform 0.1s ease-out' }}>
            ...
            <span className="text-[10px] text-slate-400 block mt-2">{t('rfZoomHint')}</span>
          </div>
        )}
      </div>
        ...
        <div className="modal-centered fixed inset-0 z-[150] flex items-center justify-center p-3 pb-[84px] sm:pb-4 bg-slate-900/50 backdrop-blur-sm"
          ...
          <div className="w-full max-w-md bg-white rounded-3xl p-5 space-y-4 max-h-[85vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-slate-700 flex items-center gap-1.5"><UserPlus size={15} className="text-teal-500" /> {t('rfAddParticipant')}</p>
              <button onClick={() => setAddOpen(false)} className="p-1.5 rounded-xl hover:bg-slate-100 text-slate-400"><X size={18} /></button>
            </div>
            <AddPanel participantList={participantList} nonParticipants={nonParticipants} t={t}
        ...
        <div className="modal-centered fixed inset-0 z-[150] flex items-center justify-center p-3 pb-[84px] sm:pb-4 bg-slate-900/50 backdrop-blur-sm"
          ...
          <div className="w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="relative">
              ...
              <RulesCard settings={settings} t={t} />
            </div>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <SettingsModal settings={settings} saving={saving}
          ...
          addRule={settingsMgr.addRule} updateRule={settingsMgr.updateRule} removeRule={settingsMgr.removeRule} t={t} />
      )}
    </div>
  );
};
