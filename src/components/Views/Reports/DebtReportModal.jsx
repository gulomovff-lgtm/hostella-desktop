...
const DebtReportModal = ({
    report, hostelName, hostelLabel,
    periodNet = 0, periodLabel = '', onClose, lang = 'ru',
}) => {
    ...
    const SOURCES = [
        { key: 'guest',    title: t('drmGuests'),    hint: t('drmGuestsHint') },
        { key: 'rental',   title: t('drmRentals'),   hint: t('drmRentalsHint') },
        { key: 'contract', title: t('drmContracts'), hint: t('drmContractsHint') },
    ];

    const label = hostelLabel || t('drmAllHostels');
    ...
    const printArgs = { report, hostelLabel: label, hostelName, periodLabel, periodNet, expected, lang };
    ...
    const CARDS = [
        { label: t('drmCardTotalDebts'), val: money(totals.debt),     sub: t('drmDebtorsCount').replace('{n}', totals.count), Icon: Wallet,        grad: 'from-rose-500 to-pink-600',    shadow: 'rgba(244,63,94,0.35)' },
        { label: t('drmCardTransfer'),   val: money(totals.transfer), sub: t('drmCardTransferSub'),   Icon: ArrowRightLeft, grad: 'from-sky-500 to-indigo-600',   shadow: 'rgba(59,130,246,0.35)' },
        { label: t('drmCardRegular'),    val: money(totals.regular),  sub: t('drmCardRegularSub'),    Icon: Banknote,      grad: 'from-amber-500 to-orange-600', shadow: 'rgba(245,158,11,0.35)' },
        { label: t('drmCardExpected'),   val: money(expected),        sub: t('drmCardExpectedSub'),   Icon: TrendingUp,    grad: 'from-emerald-500 to-teal-600', shadow: 'rgba(16,185,129,0.35)' },
    ];
            ...
            <div className="bg-slate-50 w-full sm:max-w-5xl sm:rounded-3xl shadow-2xl min-h-full sm:min-h-0 sm:max-h-[92vh] flex flex-col">
                ...
                <div className="flex items-center justify-between gap-3 px-5 py-4 bg-white border-b border-slate-200 sm:rounded-t-3xl shrink-0">
                    <div className="min-w-0">
                        <h2 className="text-lg font-black text-slate-800 truncate">{t('dvReportHeading')}</h2>
                        <p className="text-xs text-slate-400 font-semibold mt-0.5 truncate">
                            {label} · {new Date().toLocaleDateString('ru')}
                        </p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button onClick={handleExcel} disabled={busy}
                            ...
                            <Download size={15} /> {busy ? t('drmGenerating') : 'Excel'}
                        </button>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
                    ...
                    <p className="text-[11px] text-slate-500 bg-white border border-slate-200 rounded-2xl px-4 py-2.5 leading-relaxed">
                        {t('drmInfo1')}
                        {' '}{t('drmInfo2a')}{periodLabel ? <b>({periodLabel})</b> : null}{t('drmInfo2b')}
                        {' '}{t('drmInfo3')}
                    </p>
                    ...
                    {byHostel.length > 0 && (
                        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                            <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/70">
                                <span className="text-xs font-black uppercase tracking-wide text-slate-500">{t('drmByBranch')}</span>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-[11px] uppercase tracking-wide text-slate-400 border-b border-slate-100">
                                            <th className="text-left font-bold px-4 py-2">{t('drmColBranch')}</th>
                                            <th className="text-right font-bold px-3 py-2">{t('drmGuests')}</th>
                                            <th className="text-right font-bold px-3 py-2">{t('drmColRent')}</th>
                                            <th className="text-right font-bold px-3 py-2">{t('drmColContracts')}</th>
                                            <th className="text-right font-bold px-3 py-2">{t('drmColTransferShort')}</th>
                                            <th className="text-right font-bold px-3 py-2">{t('drmCardRegular')}</th>
                                            <th className="text-right font-bold px-4 py-2">{t('drmColTotal')}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {byHostel.map(h => (
                                        ...
                                        <tr className="bg-slate-50 font-black">
                                            <td className="px-4 py-2.5 text-slate-700">{t('total')}</td>
                    ...
                    {byEntity.length > 0 && (
                        <div className="bg-white rounded-2xl border border-slate-200 px-4 py-3">
                            <div className="text-xs font-black uppercase tracking-wide text-slate-500 mb-2">{t('drmExpectedByTransfer')}</div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                        <div className="flex gap-1.5 p-2.5 border-b border-slate-100 bg-slate-50/70 overflow-x-auto scrollbar-hide">
                            {[{ key: 'all', title: t('drmTabAll'), debt: totals.debt }, ...SOURCES.map(s => ({ ...s, debt: bySource[s.key].debt }))].map(s => {
                                const on = tab === s.key;
                                return (
                        ...
                        {shownRows.length === 0 ? (
                            <div className="p-10 text-center text-slate-400 text-sm">{t('drmNoDebts')}</div>
                        ) : (
                                    ...
                                    <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                                        <div className="min-w-0 flex-1">
                                            <div className="font-bold text-sm text-slate-800 truncate">
                                                {r.name}
                                                {r.records > 1 && <span className="ml-1.5 text-[10px] font-bold text-slate-400">{t('drmRecords').replace('{n}', r.records)}</span>}
                                                {r.completed && <span className="ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-500">{t('drmCompleted')}</span>}
                                            </div>
                                        ...
                                        <div className="text-right shrink-0">
                                            ...
                                            <div className="text-[10px] text-slate-400 tabular-nums">{t('drmPaidOf').replace('{paid}', money(r.paid)).replace('{charged}', money(r.charged))}</div>
                                        </div>
                                        <span className={`shrink-0 text-[10px] font-bold px-2 py-1 rounded-full ${r.method === 'transfer' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>
                                            {r.method === 'transfer' ? (r.entities?.[0] || t('drmTransfer')) : t('drmCash')}
                                        </span>
                                    </div>
                                ))}
                            </div>
