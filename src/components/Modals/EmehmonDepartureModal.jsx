...
const PAY_TYPES = [
  ...
  { value: '3', labelKey: null, literal: 'Humo' },
  { value: '4', labelKey: null, literal: 'Uzcard' },
  { value: '5', labelKey: 'emmPayTransfer' },
  { value: '6', labelKey: 'emmPayContract' },
];

const EmehmonDepartureModal = ({ guests = [], onClose, onConfirm, lang = 'ru' }) => {
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2 font-black text-slate-800">
            <Plane size={18} className="text-rose-600" /> {bulk ? `${t('emmDepartTitle')} (${list.length})` : t('emmDepartTitle')}
          </div>
        ...
        <div className="p-5 space-y-4">
          {bulk ? (
            <div className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm max-h-40 overflow-y-auto space-y-1">
              {list.map((g, i) => (
                <div key={g.id || g.passport || i} className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-slate-400 shrink-0">{g.roomNumber ? `${t('roomShortLabel')} ${g.roomNumber}` : ''}{g.passport ? ` · ${g.passport}` : ''}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm">
              ...
              <div className="text-xs text-slate-500 mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
                {guest?.roomNumber && <span>{t('roomShortLabel')} {guest.roomNumber}</span>}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wide">{t('emmAmountLabel')}</label>
              <input className={inp} value={amount} onChange={e => setAmount(e.target.value)} inputMode="decimal" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wide">{t('emmPayType')}</label>
          ...
          <label className="flex items-center gap-2 text-sm font-bold text-slate-600 cursor-pointer select-none">
            ...
            <Printer size={15} className="text-slate-400" /> {t('emmPrintDepartSheet')}
          </label>
          <p className="text-xs text-slate-400 -mt-2">
            {print ? t('emmPrintHintOn') : t('emmPrintHintOff')}
          </p>
        </div>

        <div className="flex gap-2 px-5 py-4 border-t border-slate-100">
          <button onClick={onClose} disabled={busy} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-500 font-bold text-sm hover:bg-slate-50 disabled:opacity-50">{t('cancel')}</button>
          <button onClick={submit} disabled={busy}
            ...
            <Plane size={16} /> {busy ? t('emmDepartingShort') : t('emmDepartBgBtn')}
          </button>
        </div>
      </div>
    </div>
